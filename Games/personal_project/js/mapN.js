...
setInterval(removePokemons, 9900)
...
function addPokemon() {
    ...
    // Grass pokemon
    for (let w = 0; w < 3; w++) {
        L.geoJSON(general.listOne[waterPointRandom], {
        // hier wordt een marker toegevoegd aan de geoJson gemaakte locatie
        pointToLayer: function (feature, latIng) {
        ...
        L.geoJSON(general.listTwo[waterPointRandom2], {
        // hier wordt een marker toegevoegd aan de geoJson gemaakte locatie
        pointToLayer: function (feature, latIng) {
    ...
    for (var i = 0; i < 3; i++) {
        ...
        var grassLessRandom = Math.floor(Math.random() * grassTypeLessImg.length);
        ...
        if (trueLocation[randomLocation] == false) {
            ...
            trueLocation[randomLocation] = true;
        } else if (trueLocation[randomLocation]) {
            trueLocation[randomLocation] = false;
            randomLocation+3;
            var objectTest = {"type":"Feature","properties":{},"geometry":{"coordinates":[general.listCoordinates[randomLocation][0], general.listCoordinates[randomLocation][1]],"type":"Point"}}
        } else {
            var objectTest = {"type":"Feature","properties":{},"geometry":{"coordinates":[general.listCoordinates[randomLocation][0], general.listCoordinates[randomLocation][1]],"type":"Point"}}
        ...
        onEachFeature: function (feature, layer) {
            const name = feature.properties.name || grassTypeLessNames[grassLessRandom];
            const info = feature.properties.popupContent || "fight"
            layer.bindPopup(`<strong>${name}</strong><br><a href="/fight/${grassTypeLessID[grassLessRandom]}">${info}</a>`);
        }
        }).addTo(map);
    }
...
function removePokemons() {
    ...
    for (let i =0; i < oldGrass.length; i++) {
        oldGrass[i].remove();
...
//             console.log("random:" + grassLessRandom);
//             var bigGeneralIcons = L.icon({
//                 iconUrl: grassTypeLessImg[grassLessRandom],
//                 iconSize: [60, 60],
//             });
//             var objectTest = {"type":"Feature","properties":{},"geometry":{"coordinates":[general.listCoordinates[w][0], general.listCoordinates[w][1]],"type":"Point"}}
//             L.geoJSON(objectTest, {
//             // hier wordt een marker toegevoegd aan de geoJson gemaakte locatie
//             pointToLayer: function (feature, latIng) {
//                 return L.marker(latIng, {icon:bigGeneralIcons});
//             },
//             onEachFeature: function (feature, layer) {
//                 const name = feature.properties.name || grassTypeNames[1];
//                 const info = feature.properties.popupContent || "fight"
//                 layer.bindPopup(`<strong>${name}</strong><br><a href="/fight/${grassTypeID[randomTwo]}">${info}</a>`);
//             }
//             }).addTo(map);
